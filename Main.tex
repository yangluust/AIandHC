...
The additional component of the AI shock is captured by an increase in the high-sector cutoff from $h_H$ to $h_H'>h_H$ in period 2 (see \eqref{eq:xAI_expand}).
 
An increase in the period-2 high-sector cutoff raises the period-1 human capital thresholds required to access the high sector with any effort level $e \in \{e_L, e_H\}$. Specifically, the upper bounds for non-learners ($\underline{h}_H(y)$), full-time learners ($\overline{h}_H(y)$), and part-time learners ($h_H\frac{1}{1-\delta}$) all rise by $(h_H' - h_H)\frac{1}{1-\delta}$.
...
\paragraph{Churning effect on human capital investment and labor supply}
Compared to the baseline $\gamma$-only shock (Proposition~\ref{prop:AI_human_capital}), the introduction of the $h_H'$ component operates mainly through a \textit{churning} effect. Specifically, it shifts some high-sector households with $h_H\frac{1}{1-\delta}<h<h_H'\frac{1}{1-\delta}$ from being non-learners to becoming part-time or full-time learners. It also reclassifies households with $h_M\frac{1}{1-\delta} \leq h < h_H\frac{1}{1-\delta}$ across different learner types, turning some part-time learners into full-time learners if $h_H'$ is still attainable, and moving some part-time or 
full-time learners into the non-learner group if $h_H'$ is no longer attainable.


Conditional on a given learner type, however, the within-type cutoffs in $z$ that determine labor supply and human capital investment remain unchanged from the baseline $\gamma$-only shock. The main effect of $h_H'$ is therefore to modify which learner type applies to each household.
