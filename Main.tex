...
\bigskip
\noindent
Households with $h_M\frac{1}{1-\delta} \leq h<h_H\frac{1}{1-\delta}$ have a decision rule analogous to those with $h<h_M\frac{1}{1-\delta}$. Conditional on $y$, the boundaries for learner type change to:
...
The AI shock increases sectoral labor productivity for the low and high sectors in period 2, while leaving the middle sector's labor productivity unchanged. Its effect on saving can be analyzed as if we are varying the parameter $t$ in the functions $\Delta(x,a;t)$, defined in \eqref{eq:dadt_integral}, and $\Delta_{H}(x,a;t)$, defined in \eqref{eq:dadt_fulltime}. 
...
\paragraph{Human capital investment (regime reassignment)}
Raising the high-sector cutoff in period 2 shifts the feasibility boundaries for reaching the high sector with effort $e\in\{e_L,e_H\}$ from \eqref{eq:hh} to
\[
...
Relative to the baseline $\gamma$-only shock (Proposition~\ref{prop:AI_human_capital}), the $h_H'$ component therefore works primarily through a \textit{composition} effect: it reassigns households with $h_M\frac{1}{1-\delta} \leq h<h_H\frac{1}{1-\delta}$ across learner types (turning some part-time learners into full-time learners and some full-time learners into non-learners) and turns some households with $h_H\frac{1}{1-\delta}<h<h_H'\frac{1}{1-\delta}$ from non-learners into part-time or full-time learners. 

Conditional on a given learner type, however, the within-type cutoffs in $z$ that characterize labor and training choices follow the same formulae as under the baseline $\gamma$-only shock; the role of $h_H'$ is to shift which learner type applies.

\paragraph{Labor supply}
The implications for period-1 labor supply follow the same two channels emphasized above.
\textit{Via income}, households who are downgraded from high to middle absent additional investment (those with $h'(0)\in(h_H,h_H')$) face lower anticipated period-2 income than under the $\gamma$-only shock, which tends to raise their period-1 labor supply.
\textit{Via full-time training}, among households who respond by switching toward (or intensifying) full-time training to reach $h_H'$, labor supply falls mechanically because training and work compete for time.
Overall, the expansion generates heterogeneous labor-supply responses by moving households into different training regimes.
