...
\begin{equation}
h_{t+1}=y_te_t + (1-\delta_h)h_t
\end{equation}
where $\delta_h$ is human capital's depreciation rate. We interpret $y_t e_t$ as effective human-capital investment, with $y_t$ capturing a learning-ability shock.\footnote{The learning ability shock $y_t$ is likely to be correlated with the labor productivity shock $z_t$ in reality. In our quantitative analysis, we assume they are perfectly correlated to reduce the dimensionality of the state space. For readers interested in the two-period model under this perfect-correlation assumption, Appendix~\ref{appendix:decision_rule_diagram} displays the corresponding decision rule diagram.}
...
\begin{proposition}\label{prop:dadt}
    ...
    for $x<x^\ast(a,t)$, $e=e_L$ lowers saving $\Delta(x,a;t)<0$; for $x>x^\ast(a,t)$, $e=e_L$ raises saving $\Delta(x,a;t)>0$.
\end{proposition}
\begin{proof}
...
\subsubsection{An additional component: expansion of the middle sector}
The AI shock in \eqref{eq:xAI} changes sectoral productivities in period 2. 
We now introduce an additional component of the anticipated AI shock to reflect the declining demand for junior hires in many college-level roles: the high 
sector opportunity shrinks, i.e., the cutoff for high-sector employment 
rises from $h_H$ to $h_H'>h_H$. 
That is, period-2 sectoral productivity becomes:
\begin{equation}
    \label{eq:xAI_expand}
x^{AI+}(h')=\left \{ 
\begin{array}{cl}
...
\right. 
\end{equation}
The increase in the high-sector cutoff is assumed to occur only in period 2; thus, period-1 sectoral productivity remains unchanged from \eqref{eq:x}. Setting 
$h_H'=h_H$ nests the baseline AI 
shock in \eqref{eq:xAI}.

\paragraph{Human capital investment and labor supply}
Raising the high-sector cutoff in period 2 shifts the period-1 human capital cutoffs that determine the feasibility of reaching the high sector with effort $e\in\{e_L,e_H\}$. Specifically, the upper bounds for non-learner, $\underline{h}_H(y)$, full-time learner, $\overline{h}_H(y)$, and part-time learner, $h_H\frac{1}{1-\delta}$ all increase by $(h_H'-h_H)\frac{1}{1-\delta}$.

As a result, some households who would have been able to remain in the high sector under $e=0$ when the cutoff is $h_H$ (because $h'(0)>h_H$) may now fall into the middle sector when $h'(0)\in(h_H,h_H')$. For these households, the additional component reduces the payoff of $e=0$ in period 2 and creates an incentive to invest in human capital to remain in the high sector, if reaching $h_H'$ is feasible.
Conversely, for households for whom $h_H'$ becomes infeasible even under $e=e_H$, the higher cutoff reduces the return to training aimed at accessing the high sector and shifts them toward $e=0$ in period 1.

Relative to the baseline $\gamma$-only shock (Proposition~\ref{prop:AI_human_capital}), the $h_H'$ component therefore works primarily through a \textit{composition} effect: it turns some households with $h_H\frac{1}{1-\delta}<h<h_H'\frac{1}{1-\delta}$ from non-learners into part-time or full-time learners, and reassigns households with $h_M\frac{1}{1-\delta} \leq h<h_H\frac{1}{1-\delta}$ across learner types, turning some part-time learners into full-time learners, and some part-time or full-time learners into non-learners.

Conditional on a given learner type, however, the within-type cutoffs in $z$ that characterize labor supply and human capital investment remains the same as under the baseline $\gamma$-only shock; the role of $h_H'$ is to shift which learner type applies.

 

\paragraph{Saving}
The saving implications are benchmarked against the existing decomposition in terms of $\Delta(x,a;t)$ and $\Delta_H(x,a;t)$.
Households who are downgraded to the middle sector absent extra effort experience a lower anticipated period-2 income than under the $\gamma$-only shock, which attenuates the saving effects previously attributed to higher future income for high-sector households.
For households who respond by investing to remain in the high sector, the relevant saving channel depends on whether they upskill via part-time training (governed by $\Delta(x,a;t)$) or full-time training (which also loads on $\Delta_H(x,a;t)$). The key change induced by $h_H'$ is again a composition effect: more households shift from the ``stay high'' case into the ``upskill to remain high'' cases, so that the $\Delta$ and $\Delta_H$ mechanisms apply to a larger set of households than under the baseline AI shock in \eqref{eq:xAI}.
...
$\delta_h$  & 0.1  &  & HC depreciation rate & Standard value\tabularnewline
...
On the production side, we set the capital income share, $\alpha,$ to 0.36, and the depreciation rate, $\delta,$ to 0.1. For simplicity, we assume that human capital depreciates at the same rate, i.e., $\delta_h =0.1$.
