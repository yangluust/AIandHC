...
\paragraph{Households who change human capital investment}
According to 
...
\right. 
\end{equation}
If we set $h_H' = h_H$, the baseline AI shock in \eqref{eq:xAI} is nested as a special case .

\paragraph{Learner type cutoffs}
Increasing the high-sector cutoff in period 2 causes the period-1 human capital thresholds for each learner type -- those that determine whether reaching the high sector is feasible with an effort level $e \in \{e_L, e_H\}$ -- to rise accordingly. Specifically, the upper bounds for non-learners ($\underline{h}_H(y)$), full-time learners ($\overline{h}_H(y)$), and part-time learners ($h_H\frac{1}{1-\delta}$) all increase by $(h_H' - h_H)\frac{1}{1-\delta}$.

As a result, some households who previously would have been able to remain in the high sector under $e=0$ when the cutoff was $h_H$ (i.e., those with $h'(0)>h_H$) may now find themselves in the middle sector if $h'(0)\in(h_H, h_H')$. For these households, the higher $h_H'$ in period 2 reduces the value of choosing $e=0$ and creates an incentive to invest in human capital in order to stay in the high sector -- provided that $h_H'$ is attainable.

On the other hand, for households for whom $h_H'$ is not attainable even with maximum effort ($e=e_H$), the increased cutoff decreases the return to training aimed at accessing the high sector, pushing these households toward choosing $e=0$ in period 1 instead.

\paragraph{Churning effect}
Relative to the baseline $\gamma$-only shock (Proposition~\ref{prop:AI_human_capital}), the $h_H'$ component therefore works primarily through a \textit{churning} effect: it turns some households with $h_H\frac{1}{1-\delta}<h<h_H'\frac{1}{1-\delta}$ from non-learners into part-time or full-time learners, and reassigns households with $h_M\frac{1}{1-\delta} \leq h<h_H\frac{1}{1-\delta}$ across learner types, turning some part-time learners into full-time learners, and some part-time or full-time learners into non-learners.

\paragraph{Human capital investment and labor supply} Conditional on a given learner type, however, the within-type cutoffs in $z$ that characterize labor supply and human capital investment remains the same as under the baseline $\gamma$-only shock; the role of $h_H'$ is to shift which learner type applies.
...
\section{Effects of AI shock in two-period model}\label{appsec: AIeffects}
...
\subsubsection{Effects on saving when human capital investment is unchanged}
