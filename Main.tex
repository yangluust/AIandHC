...
\subsection{AI Shock}
We model the AI shock as an anticipated change occurring in a future period. We consider two components of the AI shock: a baseline component that changes the skill premium and an additional component that contracts access to the high sector.

\subsubsection{Baseline component: AI changes skill premium}
Suppose an AI shock is expected to boost labor productivity in both the low and high sectors in a future period, while leaving the middle sector unaffected. We capture the impact of the AI shock on sectoral productivity through the parameter $\gamma$, where $0<\gamma<1$:
\begin{equation}
    \label{eq:xAI}
x(h')=\left \{ 
\begin{array}{cl}
...
Recent labor market evidence suggests that AI exposure has led to reduced demand for junior hires in many college-level positions. To reflect this, we introduce an additional AI shock component: a contraction in opportunities to enter the high sector. Specifically, the threshold for high-sector employment in a future period increases from $h_H$ to $h_H' > h_H$, so that future sectoral productivity is given by:
\begin{equation}
    \label{eq:xAI_expand}
x^{AI+}(h')=\left\{ 
\begin{array}{cl}
...
A distinctive feature of AI-driven technological change, relative to earlier waves of computerization or industrial robotics, is that it can substitute for (or complement) a broad set of cognitive and non-routine tasks that are central to many high-skill occupations. This suggests that AI may affect the economy not only by raising productivity in particular sectors, but also by changing the sectoral classification itself. In other words, AI can effectively raise the skill requirements for what counts as “high-skill” work. In our framework, this corresponds to a shift in the cutoff in the human-capital distribution that separates the middle and high sectors. The baseline AI experiment in the paper increases productivity in the same sectors as before while leaving the remaining sector unchanged. In this counterfactual, we keep all of those baseline AI productivity changes exactly the same. The only additional change is that the human-capital cutoff between the middle and high sectors increases. We calibrate this cutoff shift so that the middle-sector population share rises by 5 percentage points, holding everything else fixed. We refer to this specification as the \textbf{AI+ experiment}.
