...
We consider a two-period consumption--saving problem within the framework of Section \ref{sec:period1} of the main paper. Following the notation established there, a household in period 1 has:
\begin{itemize}
...
\end{equation}
subject to the intertemporal budget constraint (from equation (409) of the main paper):
\begin{align}
c + \frac{c'}{1+r'} &= (1+r)a + n(wzx(h)) + \frac{w'z'x(h')}{1+r'}, \\
\text{with } h' &= ze + (1-\delta)h,
\end{align}
where $x(h')$ is the sectoral productivity function defined in equation (\ref{eq:x}) of the main paper. The productivity shock $z'$ follows a log-normal distribution: $\ln z' \sim \mathcal{N}(\rho_z \ln z, \sigma_z^2)$.
...
Assuming the household works in period 2 (so that $n'=1$), and using the optimality condition $c' = \beta(1+r')c$ from equation (\ref{eq:c'}) of the main paper, we can express the problem in terms of saving $a'$. 
...
\[
\text{Case 1: Low $x=x_L$ (low labor income / low sector),} 
\qquad
\text{Case 2: High $x=x_H$ (high labor income / high sector),} 
