...
\paragraph{Non-learners} are households with $h < \underline{h}_M(y)$. They cannot achieve $h' > h_M$ with either $e_L$ or $e_H$ level of human capital investment today. As a result, they will choose not to invest in human capital, $e=0$, and their future sectoral productivity will be $x(h')=1-\lambda$. 
...
\begin{flushleft}
\footnotesize{Conditional on a given learning ability $y$, the figure illustrates the decision rule $(n,e)$ as a function of states $(z,h,a)$ for households with $h_M \leq h<h_M\frac{1}{1-\delta}$. The human capital $h$ changes along the horizontal axis and the labor productivity shock $z$ changes along the vertical axis. The two vertical lines $\underline{h}_M(y)$ and $\overline{h}_M(y)$ defined in (\ref{eq:hm}) separate the state space into non-learners, slow learners, and fast learners. Within each learner type, labor supply and investment choices vary with $z$ through cutoffs $\overline{z}^L_{non}(a)$, $\overline{z}^L_{slow}(a)$, $\underline{z}^L_{fast}(a)$, and $\overline{z}^L_{fast}(a)$.} 
\end{flushleft}
    
\label{fig:rule-diagram}
\end{figure}

\paragraph{Decision rule diagram:}With a separate learning-ability shock $y$, the decision rule $(n,e)$ depends on the state $(z,y,h,a)$. Conditional on $y$, the cutoffs in $h$ are given by (\ref{eq:hm}). Figure \ref{fig:rule-diagram} illustrates how labor supply and investment choices vary with $(h,z)$ conditional on a given $y$.
...
For households with $h<h_M$, the current sector is low and $x(h)=1-\lambda$. Conditional on $y$, the cutoffs $\underline{h}_M(y)$ and $\overline{h}_M(y)$ continue to separate non-learners, slow learners and fast learners. Note that the threshold $h_M$ itself can lie in any of the three regions depending on $y$: $h_M<\underline{h}_M(y)$ (all $h<h_M$ are non-learners), $\underline{h}_M(y)\leq h_M<\overline{h}_M(y)$ (some $h<h_M$ are slow learners), or $h_M\geq \overline{h}_M(y)$ (some $h<h_M$ are fast learners). Since low-sector labor income is scaled by $1-\lambda$, the cutoffs in $z$ are adjusted by $\frac{1}{1-\lambda}$, i.e., $\overline{z}^L_{non}\frac{1}{1-\lambda}$, $\overline{z}^L_{slow}\frac{1}{1-\lambda}$, $\underline{z}^L_{fast}\frac{1}{1-\lambda}$, and $\overline{z}^L_{fast}\frac{1}{1-\lambda}$.
...
\begin{flushleft}
\footnotesize{With a separate learning-ability shock $y$, the figure illustrates the $(h,y)$ regions in which households can raise next-period human capital above $h_M$ or $h_H$. Dark shading indicates that $e_L$ is sufficient to cross the relevant threshold (fast learners), while light shading indicates that only $e_H$ is sufficient (slow learners).} 
\end{flushleft}
