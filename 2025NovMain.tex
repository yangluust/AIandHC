...
Building on these labor dynamics, our model examines how AI influences aggregate and distributional outcomes of the economy via its direct effects on sectoral productivity and via the endogenous response of human capital investment. To do so, we contrast transition dynamics between the benchmark model and a model with human capital fixed at the initial steady state (so that only the direct effect of AI is present).
...
Building on this largely empirical literature, our paper develops a model that explicitly allows for a trade-off between labor supply and human capital investment. In our framework, job polarization emerges as a voluntary response to AI advancements: households in the middle sector may choose to either downskill to the low sector or upskill to the high sector, while an increasing number of middle-sector households opt for full-time training to accelerate their upskilling.
