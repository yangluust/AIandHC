...
The AI shock increases the sectoral labor productivities for the low and high sectors in period 2, but leaving the middle sector's labor productivity unchanged. Its effect on saving can be analyzed using the households optimal saving problem \eqref{eq:savingprob} with varying $x'$ across sectors.

\paragraph{Households who will stay in the same sector}

We first discuss those households who need no human capital investment or on-job-training to stay in the same sector. 

For low-sector and high-sector households, the AI shock increases their period-2 labor income $x'$. This change of $x'$ is analogous to the effect of on-job-training $\Delta_{\text{on-job}}(x,a;t)$ defined in \eqref{eq:dadt_integral}. Proposition \ref{prop:dadt} shows that $\Delta_{\text{on-job}}(x,a;t)$ has opposite sign for low-skill and high-skill households. Therefore, the AI shock \textit{crowds out low-sector households' saving} and \textit{crowds in high-sector households' saving}. 
For middel-sector households, the AI shock brings no change to their incomes and saving. 

When households need full-time traning to stay in the same sector (middle or high sector), the AI shock affects their incentives to invest $e_H$. The middle-sector households have weaker incentives for full-time training so that the AI shock makes them save more. The high-sector households have stronger incentives for full-time training and in turn save less in response to the AI shock. 


\paragraph{Households who will upskill to a higher sector}
When households upskill via on-job-training, the low-sector houesholds do not change their saving as the AI shock does not alter their future productivity gain after they upskill. For the middle-sector households, the AI shock improves their future productivity gain from $\lambda$ to $(1+\gamma)\lambda$. This is equivalent to an increase of $t$ in the on-job-training effect on saving, $\Delta_{\text{on-job}}(x,a;t)$. If the on-job-training effect on saving is positive in absence of the AI shock, the AI shock will increase households' saving. If the on-job-training effect on saving is negative in absence of the AI shock, the AI shock may reduce or increase households' saving depending on the households current sectoral produtivity and $\gamma$.

When households upskill via full-time training, 



\paragraph{Households who will downskill to a lower sector}
They must have $e=0$. 

\paragraph{Low sector households:}
For households who will stay in the low sector in period 2, the AI shock increases their period-2 labor income $x'$ from $1-\lambda$ to $1-(1-\gamma)\lambda$. This change of $x'$ is analogous to the effect of on-job-training $\Delta_{\text{on-job}}(x,a;t)$ defined in \eqref{eq:dadt_integral}.  

\paragraph{Middle sector households:}
For households who will stay in the middle sector in period 2, the AI shock brings no change to their incomes and saving. 

For households who will work in the high sector in period 2 (via human capital investment)


\paragraph{High sector households:}

\paragraph{On-job-training:}
For the low-skill households, AI increases $x$ but reduces $t$. Hence, AI reduces the net-crowding-out effect for the low-skill households.

For the middle-skill households, AI does not change $x$ but increases $t$. Hence, AI enhances the net-crowding-in effect for the middle-skill households.

For the high-skill households, $t=1$, AI increases $x$ and therefore saving via the conventional channels. 

\paragraph{Full-time training:}
...
\subsection{Cutoffs formulae for households with $h_M\frac{1}{1-\delta} \leq h<h_H\frac{1}{1-\delta}$}
...
\subsection{Proof of another proposition}

$a'^*(x,a;t)$ is convex in $t$:
\begin{equation}
...
Increasing $t$ by $\Delta t$ changes the total effect of on-job-training on saving by 
\begin{equation}
    \int_t^{t+\Delta t} \frac{\partial a'^\star}{\partial u}(x,a;u) du .
\end{equation}
