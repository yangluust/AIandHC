...
\textbf{Non-learners} are households with $h < \underline{h}_M(y)$. They cannot achieve $h' > h_M$ with either $e_L$ or $e_H$ level of human capital investment today. \textbf{Slow learners} are households with $h \in (\underline{h}_M(y), \overline{h}_M(y))$. These households can reach $h' > h_M$ in the next period only by investing $e = e_H$ today. \textbf{Fast learners} are households with $h > \overline{h}_M(y)$. They can achieve $h' > h_M$ in the next period if they invest $e = e_L$ today. 
...
\paragraph{Slow learners} choose between $e=0$ and $e=e_H$, since $e=e_L$ incurs a cost without any future benefit. Slow learners must trade off between working and human capital investment: choosing $e=e_H$ requires not working today ($n=0$), while opting to work means forgoing investment in human capital ($n=1, e=0$).\footnote{The choice between $(n=0,e=e_H)$ and $(n=0,e=0)$ does not depend on $z$. For $e_H$ to be relevant, $\lambda$ must be large enough so that $(n=0,e=e_H)$ is preferred to $(n=0,e=0)$. See Appendix~\ref{app:cutoff_ranking} for details on the lower bound for $\lambda$.}

Slow learners prefer $(n=1,e=0)$ to $(n=0,e=e_H)$ if and only if:
\begin{align} 
    z \geq \overline{z}^L_{slow}(a,h)=\frac{(\exp(\frac{\chi_n-\chi_e e_H}{1+\beta})-1)[(1+r)a+\frac{w'z'}{1+r'}]+\lambda\frac{w'z'}{1+r'}}{w\left(1-\mathbbm{1}_{h < h_M}\lambda\right)} \label{eq:z-slow}
\end{align}

\paragraph{Fast learners} do not exert high effort $e_H$ in human capital investment. They choose among three options: $(n=1,e=0)$, $(n=1,e=e_L)$, and $(n=0,e=e_L)$,\footnote{Similar to the case of slow learners, the choice between $(n=0,e=e_L)$ and $(n=0,e=0)$ does not depend on $z$. Moreover, since our model is set up so that $(n=0,e=e_H)$ dominates $(n=0,e=0)$, it implies that $(n=0,e=e_L)$ dominates $(n=0,e=0)$.} with the decision rule as follows:
\begin{equation}
n(z,h,a),e(z,h,a) =\left \{ 
\begin{array}{cl}
n=1,e=0  & \text{ if } z \geq \overline{z}^L_{fast}(a,h) \\
n=1,e=e_L & \text{ if } \underline{z}^L_{fast}(a,h) \leq z<\overline{z}^L_{fast}(a,h) \\
n=0,e=e_L & \text{ if } z<\underline{z}^L_{fast}(a,h)
\end{array}%
\right.
\end{equation}
where 
\begin{align}
\overline{z}^L_{fast}(a,h)=\frac{\left\{\exp(\frac{\chi_e e_L}{1+\beta})\lambda\left[\exp(\frac{\chi_e e_L}{1+\beta})-1\right]^{-1}-1\right\}\frac{w'z'}{1+r'}-(1+r)a}{w\left(1-\mathbbm{1}_{h < h_M}\lambda\right)}  \label{eq:z-fast-upper}
\end{align}
\begin{align}
\underline{z}^L_{fast}(a,h)=\frac{(\exp(\frac{\chi_n}{1+\beta})-1)[(1+r)a+\frac{w'z'}{1+r'}]}{w\left(1-\mathbbm{1}_{h < h_M}\lambda\right)} \label{eq:z-fast-lower}
\end{align}
We set up our model so that $\overline{z}^L_{fast}(a,h)>\underline{z}^L_{fast}(a,h)$.\footnote{Appendix~\ref{app:cutoff_ranking} provides the parameter restrictions such that the condition for $(n=0,e=e_H)$ to dominate $(n=0,e=0)$ is sufficient for $\overline{z}^L_{fast}(a,h)>\underline{z}^L_{fast}(a,h)$.} 
...
\end{equation}
The four cutoffs in $z$ that partition the decision regions change to $\overline{z}^M_{non}(a,h)$, $\overline{z}^M_{slow}(a,h)$, $\underline{z}^M_{fast}(a,h)$, and $\overline{z}^M_{fast}(a,h)$ (see Appendix~\ref{app:cutoff_formulae} for the explicit formulae).
...
\begin{proposition}\label{prop:Delta}
    ...
    \begin{itemize}
        \item If $\Delta(x,a;t)>0$ and $t>1$, $\Delta(x,a;t') > \Delta(x,a;t)$ for $t'>t>1$. 
        \item If $\Delta(x,a;t) > 0$ and $t<1$, $\Delta(x,a;t') < \Delta(x,a;t)$ for $1>t'>t$.
    \end{itemize}
\end{proposition}
\begin{proof}
    See Appendix~\ref{appsec: proofs}.
...
\end{equation}
They separate the state space into three areas: the unshaded area represents the non-learners, the lightly-shaded area represents the slow learners, and the darkly-shaded area represents the fast learners. The areas are divided by dashed horizontal lines associated with cutoffs $\overline{z}^L_{non}(a)$, $\overline{z}^L_{slow}(a)$, $\underline{z}^L_{fast}(a)$, and $\overline{z}^L_{fast}(a)$ defined in (\ref{eq:z-non}), (\ref{eq:z-slow}), (\ref{eq:z-fast-lower}), and (\ref{eq:z-fast-upper}).
...
\end{equation}
The four cutoffs of $z$ are $\overline{z}^M_{non}$, $\overline{z}^M_{slow}$, $\underline{z}^M_{fast}$, and $\overline{z}^M_{fast}$ defined in Appendix~\ref{app:cutoff_formulae}
