...
\end{align}
We set up our model so that $\overline{z}^L_{fast}(a)>\underline{z}^L_{fast}(a)$.\footnote{Appendix provides the parameter restrictions such that the condition for $(n=0,e=e_H)$ to dominate $(n=0,e=0)$ is sufficient for $\overline{z}^L_{fast}(a)>\underline{z}^L_{fast}(a)$.} 
...
This decision rule diagram is representative for households in other four ranges of human capital. For households with $h<h_M$, $\overline{z}_M(h)$ and $\underline{z}_M(h)$ continue to be the boundaries that separate non-learners, slow learners and fast learners, but the four cutoffs are $\overline{z}^L_{non}\frac{1}{1-\lambda}$, $\overline{z}^L_{slow}\frac{1}{1-\lambda}$, $\underline{z}^L_{fast}\frac{1}{1-\lambda}$, and $\overline{z}^L_{fast}\frac{1}{1-\lambda}$.
...
When the optimal choice of human capital investment is zero, there will be no difference in optimal saving between the two senarios. When the optimal choice of human capital investment is either $e_L$ or $e_H$, we compare household's optimal saving with its counterpart when $e$ is fixed to zero, i.e., $(n=1,e=0)$.\footnote{What about $(n=0,e=0)$? Its comparison with $(n=0,e=e_H \text{ or } e_L)$ is meaningless since no labor income in either period and human capital is irrelvant. What if we compare $(n=0,e=0)$ to $(n=1,e=e_L)$? Will these choices ever share a common state space? In other words, for the state space where $(n=1,e=e_L)$, if we exogenousely fix $e=0$, will the households ever choose not to work, i.e., $n=0$? [TO BE CHECKED]}
...
\subsubsection{Effect of on-job-training on saving}
We now compare the optimal saving between $(n=1,e=e_L)$ and $(n=1,e=0)$, where $e_L$ allows households to move to a higher sector in period 2 with higher sectoral productivity $x(h')$. We normalize $(1+r') = 1$, $w' = 1$, and the period-2 productivity shock $z'$ to $\ln z' \sim \mathcal{N}(0,\sigma_z^2)$ for analytical tractability.\footnote{The normalization of $z'$ is without loss of generality since the period-2 income is scaled by $t$ that captures the effect of the mean of $z'$.} The effect of human capital investment on period-2 income becomes:
\begin{equation}
c' = a' + t x z',
\end{equation}
where we also normalize $z'$ to $\ln z' \sim \mathcal{N}(0,\sigma_z^2)$, implying
\begin{equation}
\mathbb{E}[z'] = e^{\sigma_z^2/2}, \qquad \operatorname{Var}(z') = e^{\sigma_z^2}(e^{\sigma_z^2}-1).
\end{equation}

\begin{proposition}
    For $x=x_L<x^\ast$, $e=e_L$ lowers saving; for $x=x_H>x^\ast$, $e=e_L$ raises saving.
\end{proposition}


\subsubsection{Effect of full-time training on saving}
We next compare the optimal saving between $(n=0,e=e_L \text{ or } e_H)$ and $(n=1,e=0)$.

\section{The Effects of an Anticipated Period-2 AI Shock}
