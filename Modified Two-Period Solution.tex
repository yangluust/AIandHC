...
\]
Then equation \eqref{eq:dsdlambda_total} can be written as
\[
\boxed{\;
\frac{\partial s^\star}{\partial\lambda}(x;\lambda)
...
\subsection*{Implications for $\frac{\partial s^\star}{\partial \lambda}$}

The sign of $\frac{\partial s^\star}{\partial\lambda}$ is governed by
\[
...
\begin{itemize}
  ...
  \]
  implying $\frac{\partial s^\star}{\partial\lambda}(x;\lambda)<0$.
  ...
  $\frac{\partial s^\star}{\partial\lambda}(x;\lambda)>0$.
\end{itemize}

Under condition above, there exists a unique threshold $x^\ast(\lambda)$ at which the sign flips:
\[
\boxed{\;
\frac{\partial s^\star}{\partial\lambda}<0\ \text{for }x<x^\ast,
\qquad
\frac{\partial s^\star}{\partial\lambda}>0\ \text{for }x>x^\ast.
\;}
\]
